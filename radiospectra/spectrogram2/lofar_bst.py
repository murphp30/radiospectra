...
class LOFAR_BST:
    ...
    Optional arguments: trange = time range (sunpy.time.TimeRange),
                        ...
                        (2 element list/array with start and end frequency as astropy.units.Quantity e.g [10,90]*u.MHz),
        ...
        if self.trange is None:
            if len(file_split := self.bst_file.split("_bst")) > 1:
                ret = ax.imshow(data, aspect="auto",
                                extent=[self.times.plot_date[0], self.times.plot_date[-1], self.freqs.value[-1],
                                        self.freqs.value[0]],
                                **imshow_args)
            else:
                ret = ax.imshow(data, aspect="auto",
                                extent=[self.obs_start.plot_date, self.obs_end.plot_date, self.freqs.value[-1],
                                        self.freqs.value[0]],
                                **imshow_args)

        else:
            ret = ax.imshow(data, aspect="auto",
                            extent=[self.trange.start.plot_date, self.trange.end.plot_date, self.freqs.value[-1],
                                    self.freqs.value[0]],
                            **imshow_args)
        ax.xaxis_date()
...
class LOFAR_BST_357(LOFAR_BST):
    ...
    def __init__(self, bst_file, trange=None, frange=None, integration=1):
        ...
        self.trange = trange
        self.frange = frange
        super().__init__(self.bst_file, sbs=np.arange(488), obs_mode=3, trange=self.trange, frange=None, bit_mode=8,
...
class LOFAR_Raw(LOFAR_BST):
    ...
    Required arguments: raw_file = File name,
                                            ...
                                            obs_mode = observation mode
    Optional arguments: trange = time range (sunpy.time.TimeRange),
                                            frange = frequency range (2 element list/array with start and end frequency
                                             as astropy.units.Quantity e.g [10,90]*u.MHz),
                                            bit_mode = bit mode (usually 4, 8 or 16),
    ...
    def __init__(self, raw_file, sbs, obs_mode, trange=None, frange=None, bit_mode=8, dtype=np.float32, dt=5.12e-6):
        ...
        if not self.fil_file:
            self.obs_start = Time(raw_file.split("_")[2])
        else:
            self.obs_start = Time(raw_file.split("/")[-1][:19])
        self.__get_data()
        self.obs_end = self.obs_start + (self.dt * self.data.shape[1])

    def __get_data(self):
        nbeamlets = len(self.sbs)
        if self.fil_file:
            import sigpyproc as spp
            reader = spp.FilReader(self.raw_file)
            header_len = reader.header["hdrlen"]
        else:
            header_len = 0
        if self.trange is not None:
            ...
            count = int(count)
            data = np.memmap(self.raw_file, self.dtype, shape=count, offset=offset, mode="c").reshape(-1, nbeamlets)

        else:
            data = np.memmap(self.raw_file, self.dtype, offset=header_len, mode="c").reshape(-1, nbeamlets)
...
class LOFAR_Raw_357(LOFAR_Raw):
    ...
    def __get_data(self):
        ...
        obs_mode = np.array((3, 5, 7))
        # blank_obs_mode = np.array((3,5,5,7))
        freqs = np.array([sb_to_freq(sb, mode) for sb, mode in zip(self.sbs, obs_mode)])
        # blank_freqs = np.array([sb_to_freq(sb,mode) for sb,mode in zip(blank_sbs, blank_obs_mode)])
        ...
        # self.freqs=np.concatenate((freqs[0], blank_freqs[0], blank_freqs[1], freqs[1],
        # 							blank_freqs[2], blank_freqs[3], freqs[2]))
        self.freqs = np.concatenate((freqs[0], freqs[1], freqs[2]))
